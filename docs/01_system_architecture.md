...
* RRT* for high-level motion planning.
...
It is responsible for tracking the reference trajectory generated by the motion planner. Feedback Linearization is used to compensate for the nonlinear translational dynamics. This loop generates:
* Total thrust command $U_1$.
* Reference attitude angles $\phi_d,\ \theta_d,\ \psi_d$.
...
$$ \mathrm{RRT}^* \rightarrow \mathrm{Reference\ Trajectory} \rightarrow \mathrm{Position\ Controller\ (FL)} \rightarrow \mathrm{Safe\ RL-NMPC} \rightarrow \mathrm{Plant\ Model\ (Quadrotor)} $$
...
5. Generate collision-free reference trajectories using the $\mathrm{RRT}^*$ algorithm.
